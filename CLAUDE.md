...
**Release asset inventory** — the `publish` job refuses to flip the draft live unless the release carries exactly these six, and any change to the list must land in the workflow's upload + required-asset steps and in `test/linux-client-workflow.test.js` together:
...
3. For DMG-eligible changes: `gh workflow run "release (macOS + Windows + Linux)" -f version=X.Y.Z` → cloud builds the DMG, the Windows zip + installer, **and** the three Linux packages (all in parallel), attaching them to the GitHub Release.
